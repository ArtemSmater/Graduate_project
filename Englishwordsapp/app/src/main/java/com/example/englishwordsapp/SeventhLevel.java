...
public class SeventhLevel extends AppCompatActivity {
    ...
    private final String[] profTxt = {
            "actress", "astronaut", "ballerina",
            "clown", "dancer", "director",
            "dispatcher", "diver", "doctor",
            "musician", "nurse", "official",
            "postman", "realtor", "scientist",
            "singer", "worker"};

    private final int[] profImg = {
            R.drawable.actress, R.drawable.astronaut, R.drawable.ballerina,
            R.drawable.clown, R.drawable.dancer, R.drawable.director,
            R.drawable.dispatcher, R.drawable.diver, R.drawable.doctor,
            R.drawable.musician, R.drawable.nurse, R.drawable.official,
            R.drawable.postman, R.drawable.realtor, R.drawable.scientist,
            R.drawable.singer, R.drawable.worker};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        title.setText(getString(R.string.profession)); // this
        ...
        ivPrev.setImageResource(R.drawable.works_prev); // this
        TextView tvPrev = prevDialog.findViewById(R.id.tvPreview);
        tvPrev.setText(R.string.prof_prev);  // this
        ...
        tvEnd.setText(R.string.prof_end); // this
        // set listeners to buttons
        ...
        endDialog.findViewById(R.id.btnRight).setOnClickListener(v -> {
            Intent intent = new Intent(getApplicationContext(), EighthLevel.class);
    ...
    private void createQuestion() {
        ...
        ivTask.setImageResource(profImg[rightAns]);
        ivTask.startAnimation(animAlpha);

        // set text to buttons
        for (int i = 0, j = 0; i < buttons.length; i++) {
            if (i == rightPos) {
                buttons[i].setText(profTxt[rightAns]);
                buttons[i].startAnimation(animAlpha);
            } else {
                buttons[i].setText(profTxt[wrongPositions.get(j)]);
                buttons[i].startAnimation(animAlpha);
                j++;
            }
        }
    ...
    public void onClickAnswer(View view) {
        ...
        if (tag == rightPos) {
            ...
            Toast.makeText(getApplicationContext(), "Wrong answer!\nRight answer is " +
                    profTxt[rightAns], Toast.LENGTH_SHORT).show();
        }
        tvPoints.setText(String.format(Locale.getDefault(), "%d points", points));
        checkWon();
        createQuestion();
