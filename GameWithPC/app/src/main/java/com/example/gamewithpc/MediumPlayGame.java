...
public class MediumPlayGame extends AppCompatActivity {
    ...
    private void checkPlayer(ImageView image) {
        ...
        if (!Arrays.toString(boxPositions).contains("0") && !checkResult()) {
            ResultDialog dialog = new ResultDialog(MediumPlayGame.this,
                    "Draw!", MediumPlayGame.this);
    ...
    private void computerTurnX() {
        int position = -1;
        for (int[] array : combinations) {
            if (boxPositions[array[0]] == 0 && boxPositions[array[1]] == changePlayer() && boxPositions[array[2]] == changePlayer()) {
                position = array[0];
                boxPositions[position] = activePlayer;
                fillXBox(images[position]);
                break;
            } else if (boxPositions[array[0]] == changePlayer() && boxPositions[array[1]] == 0 && boxPositions[array[2]] == changePlayer()) {
                position = array[1];
                boxPositions[position] = activePlayer;
                fillXBox(images[position]);
                break;
            } else if (boxPositions[array[0]] == changePlayer() && boxPositions[array[1]] == changePlayer() && boxPositions[array[2]] == 0) {
                position = array[2];
                boxPositions[position] = activePlayer;
                fillXBox(images[position]);
                break;
    ...
    private void endGame() {
        if (activePlayer == 1) {
            ResultDialog dialog = new ResultDialog(MediumPlayGame.this,
                    secondName.getText().toString() + " is a winner!", MediumPlayGame.this);
            dialog.setCancelable(false);
            dialog.show();
            scoreX++;
            secondScore.setText(String.format(Locale.getDefault(), "%d", scoreX));
            ...
            ResultDialog dialog = new ResultDialog(MediumPlayGame.this,
                    firstName.getText().toString() + " is a winner!", MediumPlayGame.this);
            dialog.setCancelable(false);
            dialog.show();
            scoreO++;
            firstScore.setText(String.format(Locale.getDefault(), "%d", scoreO));
